...
export default function ClearUpdateForm(props) {
  const {
    id: idProp,
    clear,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = clearRecord
      ? { ...initialValues, ...clearRecord }
      : initialValues;
  ...
  const [clearRecord, setClearRecord] = React.useState(clear);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Clear, idProp) : clear;
      setClearRecord(record);
    };
    queryData();
  }, [idProp, clear]);
  React.useEffect(resetStateValues, [clearRecord]);
  const validations = {
    title: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Clear.copyOf(clearRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ClearUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
