...
export default function AvailabilityUpdateForm(props) {
  const {
    id: idProp,
    availability,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = availabilityRecord
      ? { ...initialValues, ...availabilityRecord }
      : initialValues;
  ...
  const [availabilityRecord, setAvailabilityRecord] =
    React.useState(availability);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Availability, idProp)
        : availability;
      setAvailabilityRecord(record);
    };
    queryData();
  }, [idProp, availability]);
  React.useEffect(resetStateValues, [availabilityRecord]);
  const validations = {
    user: [],
    startTime: [],
    endTime: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Availability.copyOf(availabilityRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AvailabilityUpdateForm")}
      {...rest}
    >
      <TextField
        label="User"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || availability)}
